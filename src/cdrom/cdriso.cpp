...
long CDR_open(void) {
	...
	char tmp[256];
	memset(tmp, 0, 256);
	strncpy(tmp, IsoFile, strlen(IsoFile) - 4);
	strcat(tmp, ".iso");
	cdHandle = fopen(tmp, "rb");
	if (cdHandle == NULL) {
		return -1;
	}
...
#ifndef DREAMCAST
#ifdef __QNXNTO__
			unsigned long long ulldirlen = (dir->size[0]&0xff) | ((dir->size[1]&0xff)<<8) |
										   ((dir->size[2]&0xff)<<16) | ((dir->size[3]&0xff)<<24) |
										   ((dir->size[4]&0xff)<<32) | ((dir->size[5]&0xff)<<40) |
										   ((dir->size[6]&0xff)<<48) | ((dir->size[7]&0xff)<<56);
#else
...
#ifndef DREAMCAST
#ifdef __QNXNTO__
			unsigned long long len = (dir->size[0]&0xff) | ((dir->size[1]&0xff)<<8) |
										   ((dir->size[2]&0xff)<<16) | ((dir->size[3]&0xff)<<24) |
										   ((dir->size[4]&0xff)<<32) | ((dir->size[5]&0xff)<<40) |
										   ((dir->size[6]&0xff)<<48) | ((dir->size[7]&0xff)<<56);
#else
