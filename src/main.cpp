...
// NEOGEO CD
	// Load BIOS
#ifdef __QNXNTO__
	FILE *fp = fopen(BIOS_PREFIX "neocd.bin", "rb");
#else
...
#ifdef __QNXNTO__
	mkdir(BASE_PREFIX, 0777);
	mkdir(ROM_PREFIX, 0777);
	mkdir(BIOS_PREFIX, 0777);
	mkdir(MEMCARD_PREFIX, 0777);
	mkdir("shared/misc/NEOCD/save/", 0777);
#endif
